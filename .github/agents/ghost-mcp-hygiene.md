...
2. E2E safety. No test targets a real/production host or hardcodes credentials. The live E2E takes real Admin credentials from the environment (never committed) and stays behind `GHOST_DEVELOPMENT` (default off, fails closed); it does not run in CI. Oversized/edge inputs are written to a local file, not passed as huge tool args.
3. Version sync. If `package.json` version changed, `package-lock.json` (root and `packages[""]`) matches — the version-sync test enforces this, and release-please normally drives the bump. If `src/` changed but the version did not, that's fine (release-please bumps on merge).
4. Commits. Conventional style (`feat`/`fix`/`refactor`/`test`/`docs`/`chore` + scope) with detailed bodies, and NO AI/Copilot/assistant attribution or `Co-Authored-By` trailers.
